...
{
    public class TransferNodeOutput : Node
    {
        public string sourceNodeName = string.Empty;
        public Guid sourceNodeId = Guid.Empty;
        public Guid sourceVariableId = Guid.Empty;

        public TransferNodeOutput() : base()
        {
            NodeName = "FROM";
        ...
        public override void Initialize()
        {
            ...
            Outputs[0].OnConnected += TransferNodeOutput_OnConnected;
        }

        private void TransferNodeOutput_OnConnected(Variable variable)
        {
            if (variable.IsConnected)
            {
                if (NodeEngine != null)
                {
                    var SourceNode = NodeEngine.Nodes.Where(x => typeof(TransferNodeInput) == x.GetType()).FirstOrDefault();
                    if (SourceNode != null)
                    {
                        sourceNodeId = SourceNode.Id;
                        sourceNodeName = SourceNode.Name;
                        sourceVariableId = SourceNode.Inputs.FirstOrDefault().Id;
                        variable.Name = SourceNode.Inputs.FirstOrDefault().Name;
                    }
                }
            }
        }
        ...
        {
            base.Loop();
            if (NodeEngine != null && sourceVariableId!=Guid.Empty)
                Outputs[0].Value = NodeEngine.GetInput(sourceVariableId).Value;
